...
This repo (**Open War**) is a fork of OpenFront.io. The game code is unchanged
from upstream; what differs is that upstream's `proprietary/` assets (logo,
favicon, display font, background music) are All-Rights-Reserved and were not
carried over — see `NOTICE.md`. Branding lives in `resources/images/`, the
display font is Overpass, and `BACKGROUND_MUSIC_TRACKS` in
