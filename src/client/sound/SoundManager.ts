...
/** Asset paths of the background tracks, played in order and then looped. */
export const BACKGROUND_MUSIC_TRACKS: string[] = [];
...
export class SoundManager {
  ...
  constructor(eventBus: EventBus, userSettings: UserSettings) {
    ...
    this.safely("initialize background music", () => {
      // No background music ships with this project — upstream's tracks are
      // proprietary and were not carried over. Drop your own files into
      // resources/sounds/music/ and list them here; everything else (volume
      // slider, track rotation, sound effects) already works.
      this.backgroundMusic = BACKGROUND_MUSIC_TRACKS.map(
        (track) =>
          new Howl({
            src: [assetUrl(track)],
            loop: false,
