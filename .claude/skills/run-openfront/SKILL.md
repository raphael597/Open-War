...
If the machine already has a Chromium and the download is blocked (proxied
or offline network — `setup.sh` then fails with `403 host not permitted`),
point the driver at the existing binary instead:

```bash
export OPENFRONT_CHROME=/path/to/chrome-linux/chrome
```
...
WebGL works headless via SwiftShader, and the screenshots show the real
rendered map — but `initGL` deliberately rejects a software context
(`GLUnavailableError: WebGL2 unavailable: software`, the game is ~1fps on
it), so on a GPU-less machine you must pass `spoofHardwareGL: true` to
`launch()`. It drops `failIfMajorPerformanceCaveat` and reports a hardware
renderer string; rendering stays correct, just slow.
...
// throttle is REQUIRED in-game, see below; spoofHardwareGL on GPU-less hosts
const { browser, page } = await launch({
  rafIntervalMs: 3000,
  spoofHardwareGL: true,
});
await gotoHome(page);
await openSoloModal(page);
